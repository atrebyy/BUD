...
def register():
    if request.method == "POST":
        ...
        db.execute(
            "INSERT INTO user_profile (first_name, middle_name, last_name, username, email, password_hash) VALUES (?, ?, ?, ?, ?, ?);",
            first_name,
            middle_name,
            last_name,
            username,
...
def delete():
    if request.method == "POST":
        ...
        db.execute(
            "DELETE FROM user_profile WHERE email =? AND username=?",
            email,
            session["username"],
        )
        session.clear()
...
def profile():
    user = db.execute(
        "SELECT * FROM user_profile WHERE username=?;", session["username"]
    )
...
def edit():
    if request.method == "POST":
        ...
        db.execute(
            "UPDATE user_profile SET first_name=?, middle_name=?, last_name=?, username=?, email=? WHERE username=?;",
            new_first_name,
            new_middle_name,
            new_last_name,
            new_username,
...
def change_password():
    if request.method == "POST":
        ...
        if new_password == current_password:
            flash(
                "Please choose a new passowrd that is different from you current password."
            )
            return redirect("/change_password")

        if not is_strong_password(new_password):
            flash(
                "Your new password must be at least 8 characters long and include at least one uppercase letter, one lowercase letter, one digit, and one special character."
            )
            return redirect("/change_password")
        ...
        db.execute(
            ...
            generate_password_hash(new_password, method="sha256"),
            session["username"],
        )
...
def dashboard():
    ...
    if request.method == "POST":
        ...
        if option == options[0]:
            ...
            if action in inorout:
                db.execute(
                    "INSERT INTO cash (username, action, description, amount) VALUES (?, ?, ?, ?);",
                    session["username"],
                    action,
                    description,
                    amount,
        ...
        if not name:
            ...
            current_asset = db.execute(
                "SELECT * FROM assets WHERE username=? AND name=?;",
                session["username"],
                name,
            )
            if transaction == transactions[0]:
                if not current_asset:
                    db.execute(
                        "INSERT INTO assets (username, name, description, amount) VALUES (?, ?, ?, ?);",
                        session["username"],
                        name,
                        description,
                        amount,
                ...
                if check_for_zero[0]["amount"] == 0:
                    db.execute(
                        "DELETE FROM assets WHERE username=? AND name=?;",
                        session["username"],
                        name,
                    )
            ...
            if transaction == transactions[0]:
                if not current_liability:
                    db.execute(
                        "INSERT INTO liabilities (username, name, description, amount) VALUES (?, ?, ?, ?);",
                        session["username"],
                        name,
                        description,
                        amount,
    ...
    liabilities = db.execute(
        "SELECT * FROM liabilities WHERE username=?;", session["username"]
    )
    if liabilities:
        total = 0
